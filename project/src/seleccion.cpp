...
void FijarColVertsIdent( Cauce & cauce, const int ident )  // 0 ≤ ident < 2^24
{
   ...
   const unsigned char
          rojo  = (ident)          % 0x100U
        , verde = (ident/0x100U)   % 0x100U
        , azul  = (ident/0x10000U) % 0x100U;
...
bool Seleccion( int x, int y, Escena * escena, ContextoVis & cv_dib )
{
   ...
   CamaraInteractiva * camara_interact = escena->camaraActual();
   camara_interact->activar(*cv.cauce_act);

   // 5. Visualizar el objeto raiz actual (se debe leer de la escena con 'objetoActual()')

   Objeto3D * obj_raiz_actual = escena->objetoActual();
   obj_raiz_actual->visualizarGL(cv);
   ...
   if (id == 0) {
      std::cout << "Identificador de pixel es 0; no se hace nada\n";
      return false;
   }
   ...
   if (obj_raiz_actual->buscarObjeto(id, MAT_Ident(), &target, centro)) {
      camara_interact->mirarHacia(centro);
      std::cout << "Objeto con id = " << id << " encontrado: " << target->leerNombre() << endl;
   }
   else {
      std::cout << "No se ha podido encontrar el objeto\n";
      return false;
   }
